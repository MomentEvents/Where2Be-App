...
const CreateEvent = ({ navigation, routenew }) => {
  ...
  // in case we're debugging and we want ^S to set loading to false
  useEffect(() => {setLoading(false)}, [])
  const handleSubmit = () => {
    // Check edge cases
    if(newDisplayName === "" || newDisplayName.match(/^[0-9A-Za-z]/) === null){
    ...
    if(errorString == ""){
        Alert.alert("Success", "New username is " + newDisplayName);
        setLoading(false);
