 ...
 * up the user based on the email or username. Once it validates the user, it
 * writes the token.
 * 
 * Parameters:
 *          checkUser: The user in which we will check if it exists and does have a valid hash
 ...
 * signup
 * 
 * Attempts to signup a user through creating an inputted User object into DB. Once it creates the user,
 * it creates a session id through calling createAndStoreSessionId and writes it through calling
 * writeToken.
 * 
 * Parameters:
 *          checkUser: The user in which we will check if it exists and does have a valid hash
 ...
 * writeToken
 * 
 * Attempts to write a token into the storage. It writes the session id and expiration date (typically 15 minutes after writing)
 * 
 ...
 * updateToken
 * 
 * Attempts to get the token from the storage, increments it by 15 minutes, then writes it again
 * 
 * Parameters: None
